...
export const tonKhoAPI = {
  ...
  getPhuTungInKho: async (ma_kho, ma_pt) => {
    return axiosInstance.get(`${API_ENDPOINTS.TON_KHO_KHO(ma_kho)}/phu-tung/${ma_pt}`);
  },
  ...
  getOutOfStock: async (ma_kho) => {
    return axiosInstance.get(`${API_ENDPOINTS.TON_KHO_KHO(ma_kho)}/out-of-stock`);
  },
  
  // Adjust stock (dieu chinh ton kho - kiem ke)
  adjustStock: async (ma_kho, ma_pt, data) => {
    return axiosInstance.post(`${API_ENDPOINTS.TON_KHO_KHO(ma_kho)}/adjust/${ma_pt}`, data);
  },
  ...
  getStockMovements: async (ma_kho, params) => {
    return axiosInstance.get(`${API_ENDPOINTS.TON_KHO_KHO(ma_kho)}/movements`, { params });
  }
};